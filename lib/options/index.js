...
function mergeOptions (source, target) {
  const targetGlobal = target.global
  const sourceGlobal = source.global
  const mergedOptions = Object.assign({}, source, target)
  const mergedGlobal = mergeTypeKnowledge(targetGlobal, sourceGlobal)
  if (mergedGlobal) { mergedOptions.global = mergedGlobal }
  return mergedOptions
}

function mergeOptionsCached (previousOptions, newOptions, cache) {
...
export default function getOptions (request) {
  ...
  const chainProviders = (index) => {
    ...
    return async function () {
      ...
      const cacheKey = request.editor.getBuffer().getPath()
