'use babel'

/*
 * Functions for resolving the types of AST nodes
 */

import {
  tableNew, functionNew, stringNew, numberNew, booleanNew, nilNew, unknownNew,
  tableResolve, tableSet, mergeTypeKnowledge,
  functionPushReturnType, functionGetReturnTypes,
  typeDefGet,
  KEY_RETURN, TYPE_TABLE
} from './typedefs'

export function nodeGetReturnTypes (context, nodes, returnTypes = []) {
  nodes.forEach(node => {
    switch (node.type) {
      case 'ReturnStatement':
        node.arguments.forEach((arg, index) => {
          returnTypes[index] = mergeTypeKnowledge(context, returnTypes[index], nodeGetType(context, arg))
        })
        break
      case 'IfStatement':
        node.clauses.forEach(clause => {
          nodeGetReturnTypes(context, clause.body, returnTypes)
        })
        break
      case 'ForNumericStatement':
      case 'ForGenericStatement':
      case 'WhileStatement':
      case 'RepeatStatement':
        nodeGetReturnTypes(context, node.body, returnTypes)
        break
    }
  })
  return returnTypes
}

export function identifierScope (node) {
  if (node.isLocal === true) {
    return node.scope
  }
  if (node.isLocal === false) {
    return node.globalScope
  }
  // if isLocal is undefined, it's not a symbol, but maybe an index
}

export function nodeGetType (context, node, multipleTypes) {
  if (!node || node.isPlaceholder) { return }
  const type = node.type

  if (type === 'Identifier') {
    return tableResolve(context, identifierScope(node), node.name)
  }
  if (type === 'IndexExpression') {
    if (node.index.type !== 'StringLiteral') { return }
    const baseType = nodeGetType(context, node.base)
    if (!baseType || baseType.type !== TYPE_TABLE) { return }
    return tableResolve(context, baseType, node.index.value)
  }
  if (type === 'MemberExpression') {
    if (node.identifier.type !== 'Identifier') { return }
    const baseType = nodeGetType(context, node.base)
    if (!baseType || baseType.type !== TYPE_TABLE) { return }
    return tableResolve(context, baseType, node.identifier.name)
  }
  if (type === 'NumericLiteral') { return numberNew() }
  if (type === 'StringLiteral') { return stringNew() }
  if (type === 'BooleanLiteral') { return booleanNew() }
  if (type === 'NilLiteral') { return nilNew() }
  if (type === 'FunctionDeclaration') {
    const func = functionNew()
    const args = []
    if (
      node.identifier &&
      node.identifier.type === 'MemberExpression' &&
      node.identifier.indexer === ':'
    ) {
      args.push({ name: 'self' })
    }
    node.parameters.forEach(param => {
      args.push({
        name: param.type === 'Identifier' ? param.name : `arg${args.length + 1}`
      })
    })
    func.docs = { args }
    nodeGetReturnTypes(context, node.body).forEach(returnType => {
      functionPushReturnType(context, func, returnType)
    })
    return func
  }
  if (type === 'TableConstructorExpression') {
    const table = tableNew()
    node.fields.forEach(field => {
      if (field.type === 'TableKeyString' || field.type === 'TableKey') {
        let key = null
        if (field.key.type === 'Identifier') { key = field.key.name }
        if (field.key.type === 'StringLiteral') { key = field.key.value }
        if (!key) { return }
        tableSet(context, table, key, nodeGetType(context, field.value) || unknownNew())
      }
    })
    return table
  }
  if (node.requireCache) {
    const entry = node.requireCache[node.requireValue]
    if (!entry) { return }
    const { returnTypes } = entry
    return multipleTypes ? returnTypes : returnTypes[0]
  }
  if (
    type === 'CallExpression' ||
    type === 'StringCallExpression'
  ) {
    const funcType = nodeGetType(context, node.base)
    if (!funcType || !funcType.returnCount) { return }
    if (!multipleTypes) { return typeDefGet(context, funcType, KEY_RETURN(0)) }
    return functionGetReturnTypes(context, funcType)
  }
}

export function getLValue (context, node) {
  if (!node) { return }
  const type = node.type

  if (type === 'IndexExpression') {
    if (node.index.type !== 'StringLiteral') { return }
    const baseType = nodeGetType(context, node.base)
    if (!baseType) { return }
    const key = node.index.value
    const table = tableResolve(context, baseType, key, true) || baseType
    return { table, key }
  }
  if (type === 'MemberExpression') {
    if (node.identifier.type !== 'Identifier') { return }
    if (node.identifier.isPlaceholder) { return }
    const baseType = nodeGetType(context, node.base)
    if (!baseType) { return }
    const key = node.identifier.name
    const table = tableResolve(context, baseType, key, true) || baseType
    return { table, key }
  }
  if (type === 'Identifier') {
    if (node.isPlaceholder) { return }
    const scope = identifierScope(node)
    const table = tableResolve(context, scope, node.name, true) || scope
    return { table, key: node.name }
  }
}
