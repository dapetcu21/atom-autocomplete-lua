...
import {
  ...
  tableSearch, contextCopy, typeDefClone, typeDefGet,
  TYPE_FUNCTION, KEY_ARGUMENT
} from './typedefs'
...
window.__LOG__ = window.__LOG__ || false
...
export default class Analysis {
  constructor (options, query) {
    ...
    const context = contextCopy(options.context)
    const globalScope = typeDefClone(context, options.global)
  ...
  _onScopeIdentifierName = (newName, data) => {
    ...
    if (data && data.parameterOf) {
      ...
      if (func && func.type === TYPE_FUNCTION && func.argCount) {
        const argType = typeDefGet(context, func, KEY_ARGUMENT(data.parameterIndex))
        if (argType) {
