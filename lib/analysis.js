...
export default class Analysis {
  constructor (options, query) {
    ...
    options.moduleResolver = options.moduleResolver || new ModuleResolver(options)
  ...
  _onScopeIdentifierName = (newName) => {
  ...
  _evaluate = async (syncAction) => {
    if (this.requires.size) {
      ...
      await Promise.all([...this.requires].map(async moduleName => {
        const modulePath = await this.options.moduleResolver.resolve(moduleName)
        const module = await this.options.moduleCache.get(modulePath, this._analyseModule)
  ...
  returnModule = async () => {
    return await this._evaluate(() => {
      const returnTypes = nodeGetReturnTypes(this.chunk.body)
