...
export default class LuaProvider {
  ...
  constructor () {
    ...
    window.atom.config.observe('autocomplete-lua.excludeLowerPriority', (value) => {
      this.excludeLowerPriority = value
    })
    window.atom.config.observe('autocomplete-lua.inclusionPriority', (value) => {
  ...
  getSuggestions = async function (request) {
    ...
    __LOG__ && console.log('suggestions', request)
