...
export function typeDefDeepClone (_typeDef) {
  ...
  function _deepClone (typeDef, setter) {
    ...
    switch (typeDef.type) {
      ...
      case 'function':
        if (typeDef.variants) {
          typeDef.variants.forEach((variant, vidx) => {
            if (variant.args) {
              variant.args.forEach((arg, aidx) => {
                _deepClone(arg.typeDef, v => { clone.variants[vidx].args[aidx].typeDef = v })
              })
            }
          })
        }
        if (typeDef.args) {
          typeDef.args.forEach((arg, aidx) => {
            _deepClone(arg.typeDef, v => { clone.args[aidx].typeDef = v })
          })
        }
