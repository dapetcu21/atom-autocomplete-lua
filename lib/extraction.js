...
const mergeTypeKnowledge = (oldType, newType) => {
  const oldPriority = discoveryPriority[oldType ? oldType.type : 'nil']
  const newPriority = discoveryPriority[newType ? newType.type : 'nil']
...
const discoveredType = (table, key, newType, allowOverwrite) => {
  ...
  if (mergeTypeKnowledge(oldType, newType) === newType) {
    if (isMeta) {
      tableSetMetatable(table, newType)
    } else {
      tableSet(table, key, newType)
    }
  }
}
