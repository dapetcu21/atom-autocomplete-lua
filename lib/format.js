...
const emptyVariant = {}
const emptyArray = []
function formatVariant (key, typeDef, variant, prefix, trimSelf) {
...
export default function formatResults (results, prefix, trimSelf) {
  const suggestions = []
  results.forEach(({ key, typeDef }) => {
    if (typeDef.variants) {
      typeDef.variants.forEach(variant =>
        suggestions.push(formatVariant(key, typeDef, variant, prefix, trimSelf))
      )
    } else {
      suggestions.push(formatVariant(key, typeDef, emptyVariant, prefix, trimSelf))
    }
  })
  return suggestions
}
